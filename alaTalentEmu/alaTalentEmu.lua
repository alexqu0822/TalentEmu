...
do	-- equipmentFrame
	...
	function NS.CreateEquipmentFrame(mainFrame)
		local equipmentFrameContainer = CreateFrame("FRAME", nil, mainFrame);
		equipmentFrameContainer:SetPoint("TOPRIGHT", mainFrame, "TOPLEFT", 0, 0);
		equipmentFrameContainer:SetPoint("BOTTOMRIGHT", mainFrame, "BOTTOMLEFT", 0, 0);
		equipmentFrameContainer:SetWidth(ui_style.equipmentFrameXSize);
		if config.win_style == 'ala' then
			equipmentFrameContainer:SetBackdrop(ui_style.equipmentFrameBackdrop);
			equipmentFrameContainer:SetBackdropColor(unpack(ui_style.equipmentFrameBackdropColor));
			equipmentFrameContainer:SetBackdropBorderColor(unpack(ui_style.equipmentFrameBackdropBorderColor));
		elseif config.win_style == 'blz' then
			equipmentFrameContainer:SetBackdrop(ui_style.equipmentFrameBackdrop_blz);
			equipmentFrameContainer:SetBackdropColor(unpack(ui_style.equipmentFrameBackdropColor_blz));
			equipmentFrameContainer:SetBackdropBorderColor(unpack(ui_style.equipmentFrameBackdropBorderColor_blz));
		end
		equipmentFrameContainer:Hide();
		...
		equipmentFrame.mainFrame = mainFrame;
		equipmentFrameContainer.mainFrame = mainFrame;
...
do	-- spellTabFrame
	...
	function NS.Emu_ToggleSpellTab(mainFrame)
		...
		if spellTabFrameContainer:IsShown() then
			...
			side_anchor:ClearAllPoints();
			side_anchor:SetPoint("TOPLEFT", spellTabFrameContainer, "TOPRIGHT", 2, 0);
			side_anchor:SetPoint("BOTTOMLEFT", spellTabFrameContainer, "BOTTOMRIGHT", 2, 0);
		end
	end
	...
	function NS.CreateSpellTabFrame(mainFrame)
		local spellTabFrameContainer = CreateFrame("FRAME", nil, mainFrame);
		spellTabFrameContainer:SetPoint("TOPLEFT", mainFrame, "TOPRIGHT", 0, 0);
		spellTabFrameContainer:SetPoint("BOTTOMLEFT", mainFrame, "BOTTOMRIGHT", 0, 0);
		spellTabFrameContainer:SetWidth(ui_style.spellTabFrameWidth);
		if config.win_style == 'ala' then
			spellTabFrameContainer:SetBackdrop(ui_style.spellTabFrameBackdrop);
			spellTabFrameContainer:SetBackdropColor(unpack(ui_style.spellTabFrameBackdropColor));
			spellTabFrameContainer:SetBackdropBorderColor(unpack(ui_style.spellTabFrameBackdropBorderColor));
		elseif config.win_style == 'blz' then
			spellTabFrameContainer:SetBackdrop(ui_style.spellTabFrameBackdrop_blz);
			spellTabFrameContainer:SetBackdropColor(unpack(ui_style.spellTabFrameBackdropColor_blz));
			spellTabFrameContainer:SetBackdropBorderColor(unpack(ui_style.spellTabFrameBackdropBorderColor_blz));
		end
		spellTabFrameContainer:Hide();
		local spellTabFrame = CreateFrame("FRAME", nil, spellTabFrameContainer);	-- mainFrame:GetName() .. "SpellTab"
		spellTabFrame:SetPoint("CENTER", spellTabFrameContainer);
		...
		spellTabFrame.mainFrame = mainFrame;
		spellTabFrameContainer.mainFrame = mainFrame;
		return spellTabFrame, spellTabFrameContainer;
	end
end
...
do	-- mainFrame sub objects
	...
	local function readOnlyButton_OnClick(self, button)
		...
		-- elseif button == "RightButton" then
			...
			if config.resizable_border then
				tinsert(readOnlyButton_OnClickData.elements, {
						handler = function(button, ...) config.resizable_border = false; end,
						para = {  },
				...
				tinsert(readOnlyButton_OnClickData.elements, {
						handler = function(button, ...) config.resizable_border = true; end,
						para = {  },
...
do	-- mainFrame
	local function OnSizeChanged(self, width, height)
		...
		self.spellTabFrameContainer:SetWidth(ui_style.spellTabFrameWidth * self.talentFrameScale);
